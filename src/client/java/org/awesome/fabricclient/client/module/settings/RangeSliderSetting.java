...
public class RangeSliderSetting extends Setting<double[]> {
    ...
    public double getMinValue() { return value[0]; }
    public double getMaxValue() { return value[1]; }

    public void setMinValue(double v) { value[0] = Math.max(min, Math.min(v, value[1])); }
    public void setMaxValue(double v) { value[1] = Math.max(value[0], Math.min(v, max)); }
}
